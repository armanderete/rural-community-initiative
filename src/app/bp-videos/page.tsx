...
// Removed Lottie import since we're replacing it with <video>
...
export default function Page() {
  ...
  // State to hold loaded animations (video URLs)
  const [loadedAnimations, setLoadedAnimations] = useState<any[]>([]);

  // State to hold the currently displayed animation (video URL)
  ...
  /**
   * **Dynamic Import of Videos**
   * Instead of using import() to load binary files (which requires additional loader configuration),
   * we now resolve static paths from the public folder.
   * Only 9 animations are used now.
   */
  const animationImports = useMemo(
    () => [
      () => Promise.resolve('/videos/bp-videos/animation1.mp4'),
      () => Promise.resolve('/videos/bp-videos/animation2.mp4'),
      () => Promise.resolve('/videos/bp-videos/animation3.mp4'),
      () => Promise.resolve('/videos/bp-videos/animation4.mp4'),
      () => Promise.resolve('/videos/bp-videos/animation5.mp4'),
      () => Promise.resolve('/videos/bp-videos/animation6.mp4'),
      () => Promise.resolve('/videos/bp-videos/animation7.mp4'),
      () => Promise.resolve('/videos/bp-videos/animation8.mp4'),
      () => Promise.resolve('/videos/bp-videos/animation9.mp4'),
    ],
    []
  );

  /**
   * **Effect Hook to Start Loading Videos Sequentially**
   */
  useEffect(() => {
  ...
  // State to track which video is being loaded
  ...
  useEffect(() => {
    ...
    const loadAnimation = async () => {
      try {
        const videoPath = await animationImports[loadingIndex]();
        setLoadedAnimations((prev) => [...prev, videoPath]);

        if (loadingIndex === 0) {
          setCurrentAnimation(videoPath);
        }
      } catch (error) {
  ...
  /**
   * Effect hook to initialize videos and fetch data when a wallet is connected.
   * This ensures that data fetching and video loading occur simultaneously.
   */
  useEffect(() => {
    if (address && !animationPlayed) {
      // Video loading is already handled by the above effect
      setLoading(true);
          ...
          >
            {currentAnimation && (
              <video
                ...
                onEnded={
                  !config.animationLoopSettings[currentAnimationIndex]
                    ? handleNext
                    : undefined
                }
                style={{
