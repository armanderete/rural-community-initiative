...
export default function Page() {
      ...
      const handleScoringClick = async () => {
       ...
       if (!data || data.length === 0) {
         ...
         if (insertError) {
           ...
           calculateFunding(); // Call funding calculation

           // Fetch the updated row from Supabase
           ...
           if (fetchError) {
             ...
             // Extract updated scores for milestones 1 to 8
             const updatedScores = [];
             for (let i = 1; i <= 8; i++) {
             ...
             // Recalculate funding with new scores
             calculateFunding();
           }
         }
       } else {
 ...
 const [fundingCalculations, setFundingCalculations] = useState<string[]>(Array(8).fill(''));


 const calculateFunding = () => {
   // Sum scores of active milestones
   let totalScore = 0;
   ...
   const newFundingCalculations = milestoneScores.map((score, index) => {
     const numericScore = Number(score || 0);
     if (totalScore === 0) return '$0.0'; // Avoid division by zero
     // Apply formula: (score / total) * tier * MatchingMultiplier
 ...
 const renderFundingCalculations = () => {
   ...
   for (let i = 1; i <= 8; i++) {
     ...
     const data = (MilestoneMenuButtons as any)[key];
     if (!data) continue; // Skip if no config
     ...
     elements.push(
       <div
         ...
         style={{
           ...
           color: 'white',
           fontWeight: 'bold',
           fontSize: 'calc(min(5vw, 5vh))', // Responsive font size
           zIndex: 20, // Ensure this is higher than the Lottie animation
         }}
       >
